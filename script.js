...
const fuelForm = document.querySelector('#fuel-form') || document.querySelector('.form-panel form');
const entryList = document.querySelector('#entry-list') || document.querySelector('.entry-list');
...
function renderEntries(entries) {
    ...
    entryList.innerHTML = entries.slice(0, 3).map((entry) => {
        const station = entry.station === 'shell' ? '<div class="entry-brand shell-logo">S<span>hell</span></div>' : '<div class="entry-brand orlen-logo">ORLEN</div>';
        const price = entry.price_per_liter || entry.amount / entry.liters;
        return `<article class="entry-item">${station}<div class="entry-details"><strong>${Number(entry.liters).toFixed(2)} L <span>·</span> ${entry.station === 'shell' ? 'Shell' : 'ORLEN'}</strong><small>${formatDate(entry.entry_date)}</small></div><div class="entry-amount"><strong>${formatMoney(entry.amount)} Kč</strong><small>${Number(price).toFixed(2)} Kč/L</small></div></article>`;
    }).join('');
}

function renderTotals(entries) {
    const totals = { shell: 0, orlen: 0 };
    entries.forEach((entry) => { totals[entry.station] += Number(entry.amount); });
    ...
    document.querySelector('#combined-used').innerHTML = `${formatMoney(combined)} <small>Kč</small>`;
    document.querySelector('#combined-caption').textContent = `${((combined / 10000) * 100).toFixed(1)}% of 10,000 Kč monthly limit`;
}
...
async function loadEntries() {
    ...
    const { data, error } = await supabaseClient.from('fuel_entries').select('*').gte('entry_date', monthStart).order('entry_date', { ascending: false }).order('created_at', { ascending: false });
    if (error) {
...
async function saveEntry(event) {
    ...
    const values = {
        ...
        liters: Number(document.querySelector('#liters').value),
...
fuelForm?.querySelector('.submit-button')?.addEventListener('click', saveEntry);

const themeToggle = document.querySelector('.theme-toggle');
const savedTheme = localStorage.getItem('gassy-theme');
if (savedTheme === 'dark' || (!savedTheme && window.matchMedia('(prefers-color-scheme: dark)').matches)) document.body.classList.add('dark-mode');
